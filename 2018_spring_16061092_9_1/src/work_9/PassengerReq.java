...
public class PassengerReq extends Thread{ 
	...
	public void run() {
		/** @ REQUIRES: None;
		@ MODIFIES: writer,System.out,robbingallnum,robbingalllist,
		robbingnownum,robbingnowlist,taxibest.status;
			...
			for(int i=0;i<robbingnownum;i++) {
				...
				if(taxinow.getnowstatus()!=1) {
					...
					nowbest = robbingnowlist[i];
						...
						SPFA spfa = new SPFA();
						if(spfa.distance(taxinow.getnowi()*80+taxinow.getnowj(), seti*80+setj)<spfa.distance(taxibest.getnowi()*80+taxibest.getnowj(), seti*80+setj)) {
							//比当前最佳车距离乘客出发点更近
							nowbest= robbingnowlist[i];
						}
					}
